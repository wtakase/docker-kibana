...
router.use(function (req, res, next) {
  ...
  var remote_user_field = config.kibana.remote_user_field || 'x-authenticated-user';
  var remote_use = null;
  if (options.headers[remote_user_field] && config.kibana.multiple_index) {
    var remote_user_matches = options.headers[remote_user_field].match(/(\w+)/);
    if (remote_user_matches) {
      remote_user = remote_user_matches[1];
      var re_url = new RegExp('(' + uri.protocol + '//' + uri.host + '/)(' + config.kibana.index + ')(/.*)', 'g');
      options.url = options.url.replace(re_url, "$1" + "$2" + "." + remote_user + "$3");
      logger.error({ options: options }, 'WTAKASE: replace url');
    }
  }
  ...
  if (req.rawBody) {
    ...
    if (remote_user) {
      var re_body = new RegExp('(.*_index\s*:\s*")' + config.kibana.index + '(".*)', 'g');
      options.body = options.body.replace(re_body, "$1" + "$2" + "." + remote_user + "$3");
      logger.error({ options: options }, 'WTAKASE: replace body');
    }
  } else {
